...
export default handleActions(
    {
        ...
        [CURR_ID] : (state, action) => produce(state, (draft) => {
            draft.memberId = action.payload.memberId
        }),
        [EXIT_ID] : (state, action) => produce(state, (draft) => {
            draft.memberId = draft.memberId.filter(exit => exit !== action.payload.memberId);
        }),
